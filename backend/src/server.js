...
app.get('/api/informes/horas-trabajador', authMiddleware, async (req, res) => {
  try {
    ...
    console.log(`📅 Consultando asignaciones de ${inicioMes} a ${finMes}`);

    // 🔥 CONSULTAR ASIGNACIONES (no registro_horas)
    const asignaciones = await prisma.asignacion.findMany({
      where: {
        trabajadorId: parseInt(trabajadorId),
        fecha: {
        ...
        estado: {
          notIn: ['CANCELADO'] // Excluir canceladas
        }
      },
      include: {
        centro: { 
    ...
    // Calcular horas por semana para detectar extras
    const asignacionesPorSemana = {};
    asignaciones.forEach(asig => {
      const semana = getWeekNumber(new Date(asig.fecha));
      if (!asignacionesPorSemana[semana]) {
    ...
    for (const [semana, asignacionesSemana] of Object.entries(asignacionesPorSemana)) {
      let horasAcumuladasSemana = 0;

      for (const asig of asignacionesSemana) {
        // Calcular horas de esta asignación
        const detalle = await calcularDetalleHoras(
          {
            horaInicio: asig.horaInicio,
            horaFin: asig.horaFin,
            fecha: asig.fecha
        ...
        horasAcumuladasSemana = detalle.horasAcumuladasSemana;

        // Acumular totales
        totalNormales += detalle.horasNormales;
        totalExtras += detalle.horasExtra;
        totalNocturnas += detalle.horasNocturnas;
        totalFestivas += detalle.horasFestivo;
        ...
        detallesDias.push({
          ...
          centro: asig.centro?.nombre || 'Sin centro',
          cliente: asig.centro?.cliente?.nombre || 'Sin cliente',
          horasNormales: parseFloat(detalle.horasNormales.toFixed(2)),
          horasExtras: parseFloat(detalle.horasExtra.toFixed(2)),
          horasNocturnas: parseFloat(detalle.horasNocturnas.toFixed(2)),
          horasFestivas: parseFloat(detalle.horasFestivo.toFixed(2))
        });
        ...
        semanas[key].normales += detalle.horasNormales;
        semanas[key].extras += detalle.horasExtra;
        semanas[key].nocturnas += detalle.horasNocturnas;
        semanas[key].festivas += detalle.horasFestivo;
        semanas[key].total += detalle.horasNormales + detalle.horasExtra;
      }
    }

    console.log(`✅ Totales calculados - Normales: ${totalNormales}, Extras: ${totalExtras}`);
...
app.get('/api/informes/horas-cliente', authMiddleware, async (req, res) => {
  try {
    ...
    asignaciones.forEach(asig => {
      ...
      if (registro) {
        const horas = parseFloat(registro.horasNormales) + parseFloat(registro.horasExtra);
        const costeHora = parseFloat(asig.trabajador.categoria.precioHora) || 0;
        const coste = horas * costeHora;

        porCentro[centroNombre].totalHoras += horas;
        porCentro[centroNombre].horasNormales += parseFloat(registro.horasNormales);
        porCentro[centroNombre].horasExtras += parseFloat(registro.horasExtra);
        porCentro[centroNombre].horasNocturnas += parseFloat(registro.horasNocturnas);
        porCentro[centroNombre].horasFestivas += parseFloat(registro.horasFestivo);
...
/**
 * INFORME 4: Resumen de Ausencias
 * GET /api/informes/resumen-ausencias?mes=MM&año=YYYY
 */
app.get('/api/informes/resumen-ausencias', authMiddleware, async (req, res) => {
  try {
   ...
   console.log(`🔍 Buscando ausencias entre ${inicioMes.toISOString()} y ${finMes.toISOString()}`);
...
const ausencias = await prisma.ausencia.findMany({
  where: {
    ...
    ausencias.forEach(aus => {
      const tipo = aus.tipoAusencia.nombre;
      
      if (!porTipo[tipo]) {
        porTipo[tipo] = {
          codigo: aus.tipoAusencia.codigo,
          color: aus.tipoAusencia.colorHex,
          totalDias: 0,
          cantidad: 0,
          trabajadores: []
        };
      ...
      porTipo[tipo].totalDias += aus.diasTotales;
      porTipo[tipo].cantidad++;
      porTipo[tipo].trabajadores.push({
        id: aus.trabajador.id,
        nombre: `${aus.trabajador.nombre} ${aus.trabajador.apellidos}`,
        fechaInicio: aus.fechaInicio.toISOString().split('T')[0],
        fechaFin: aus.fechaFin.toISOString().split('T')[0],
        dias: aus.diasTotales
      });

      totalDias += aus.diasTotales;
    });
    ...
    res.json({
      ...
      porTipo: Object.keys(porTipo).map(nombre => ({
        tipo: nombre,
        ...porTipo[nombre]
      }))
    });
    ...
    console.error('Error en informe ausencias:', error);
    res.status(500).json({ error: 'Error generando informe' });
  }
});
